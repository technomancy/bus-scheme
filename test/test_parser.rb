...
class BusSchemeParserTest < Test::Unit::TestCase
  ...
  def test_parses_two_strings
    assert_parses_to [:concat.sym, "hello", "world"], "(concat \"hello\" \"world\")"
  end
  ...
  def test_parse_list_of_atoms
    assert_parses_to [:+.sym, 2, 2], "(+ 2 2)"
  end

  def test_parse_list_of_atoms_with_string
    assert_parses_to [:+.sym, 2, "two"], "(+ 2 \"two\")"
  end

  def test_parse_list_of_nested_sexprs
    assert_parses_to [:+.sym, 2, [:+.sym, 2]], "(+ 2 (+ 2))"
  end

  def test_parse_list_of_deeply_nested_sexprs
    assert_parses_to [:+.sym, 2, [:+.sym, 2, [:+.sym, 2, 2]]], "(+ 2 (+ 2 (+ 2 2)))"
  end

  def test_parse_two_consecutive_parens_simple
    assert_parses_to [:let.sym, [[:foo.sym, 2]]], "(let ((foo 2)))"
  end

  def test_parse_two_consecutive_parens
    assert_parses_to [:let.sym, [[:foo.sym, 2]], [:+.sym, :foo.sym, 2]], "(let ((foo 2)) (+ foo 2))"
  end
  ...
  def test_parses_vectors
    ...
    assert_parses_to [:vector.sym, 1, 2], "#(1 2)"
    assert_parses_to [:vector.sym, 1, [2, 3, 4]], "#(1 (2 3 4))"
  end
...
#   end
  ...
  def test_floats
    ...
    assert_parses_to  0.22,".22"
    assert_parses_to  2.22, "2.220"
  end

  def test_negative_ints
    assert_parses_to -1, "-1"
    assert_parses_to 0, "-0"
    assert_parses_to -2, "-02"
  end

  def test_negative_floats
    assert_parses_to -0.22, "-0.22"
    assert_parses_to -0.22, "-.22"
    assert_parses_to -0.1, "-0.10"
  end
...
#   end
  ...
  def test_quote
    assert_parses_to [:quote.sym, :foo.sym], "'foo"
    assert_equal [:'(', :quote.sym, :'(', :foo.sym, :bar.sym, :baz.sym, :')', :')'], BusScheme::tokenize("'(foo bar baz)").flatten
    assert_parses_to [:quote.sym, [:foo.sym, :bar.sym, :baz.sym]], "'(foo bar baz)"
    assert_parses_to [:quote.sym, [:+.sym, 20, 3]], "'(+ 20 3)"
  end

  #  have to change normalize_whitespace to not turn newlines into spaces for this to work
  def test_ignore_comments
    assert_parses_to nil, ";; hello"
    assert_parses_to 12, "12 ;; comment"
    assert_parses_to [:+.sym, 2, 2], "(+ 2;; this is a mid-sexp comment
2)"
  end
  ...
  def test_parse_random_elisp_form_from_my_dot_emacs
    ...
    assert_parses_to([:let.sym, [[:'system-specific-config'.sym,
                              [:concat.sym, "~/.emacs.d/",
                               [:'shell-command-to-string'.sym, "hostname"]]]],
                      [:if.sym, [:'file-exists-p'.sym, :'system-specific-config'.sym],
                       [:load.sym, :'system-specific-config'.sym]]], lisp)
   end
  ...
  def test_parser_allows_comments_after_last_expression
    assert_tokenizes_to [2], "2 ;; comments at the end"
    assert_parses_to 2, "2 ;; comments at the end"
  end
  
  private

  def assert_parses_to(expected, actual_string)
    assert_equal expected.sexp, BusScheme.parse(actual_string)
  end

  def assert_tokenizes_to(expected, actual_string)
    assert_equal expected, BusScheme.tokenize(actual_string)
  end
